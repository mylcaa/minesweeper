    ...
    mainFrame(nullptr)
{
    ...
    connect(&HwThread, &QThread::finished, Hw, &QObject::deleteLater);
    connect(this, &MainWindow::probe_hw, Hw, &hw::main_loop);
    connect(Hw, SIGNAL(flag(unsigned int, unsigned int)), this, SLOT(HwFlag(unsigned int, unsigned int)));
    HwThread.start();
