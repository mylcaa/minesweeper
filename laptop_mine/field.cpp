...
void Field::fillField(){
    ...
    for(unsigned int row = 0; row < numRows; ++row){
        ...
        for(unsigned int col = 0; col < numCols; ++col){
            ...
            connect(field[row][col], &Square::flagged, [=](bool isMine)
                {
                    if(isMine){
                        numCorrectFlags++;
                    }else{
            ...
            connect(field[row][col], &Square::unflagged, [=](bool isMine)
                {
                    if(isMine){
                        numCorrectFlags--;
                    }else{
            ...
            connect(field[row][col], &Square::revealed, [this]()
                {
                    ...
                    if((numCols*numRows - numMines) == numRevealedSquares && numIncorrectFlags == 0){
                        ...
                        qDebug() << "WON a";
                        qDebug() << numIncorrectFlags;
                        qDebug() << numRevealedSquares;
                        victory();
                    }
            ...
            connect(this, &Field::cover_field, [=](){

                field[row][col] -> isMine = false;
                field[row][col] -> adjacentFlaggedCnt = 0;
                field[row][col] -> adjacentMineCnt = 0;
                field[row][col] -> cover_square();
...
void Field::hw_flag(unsigned int x, unsigned int y){
    field[x][y] -> right_click();
}

void Field::hw_unflag(unsigned int x, unsigned int y){
    field[x][y] -> right_click();
}

void Field::hw_reveal(unsigned int x, unsigned int y){
    field[x][y] -> left_click();
}
